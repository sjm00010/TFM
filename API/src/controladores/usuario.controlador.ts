	...
	usuario
		...
		.catch((err) => {
			res.status(500).send({
				message:
					err.message ||
					"ERROR: Se ha producido algun error mientras se creaba el usuario.",
			});
		});
};
...
export const verificarAuth = (req, res, next) => {
	if (req.get("authorization")) {
		const [usuario, pass] = Buffer.from(
		...
		Usuario.findOne({ usuario })
			...
			.catch((err) => {
				res.status(500).send({
					message:
						"ERROR: Se ha producido algun error mientras se obtenian los datos de usuario.",
				});
			});
	} else {
		res.status(400).send({
