...
export const crear = (req, res) => {
	...
	matriz
		...
		.catch((err) => {
			...
			res.status(400).send({
				message:
					"ERROR: Se ha producido algun error mientras se creaba el ejercicio.",
			});
		});
};
	...
	EjMatriz.findById(id)
		.then((data) => {
			if (!data)
				res
					.status(404)
					.send({ message: `No se encontrar el ejercicio con id ${id}` });
			else res.send(data);
		})
		.catch((err) => {
			res.status(500).send({
...
export const buscarTodos = (req, res) => {
	EjMatriz.find({}, { dificultad: 1, enunciado: 1, id: 1 })
		...
		.catch((err) => {
			res.status(500).send({
				message:
					"ERROR: Ocurrio un error mientras se recuperaban los ejercicios matriciales",
			});
		});
};
